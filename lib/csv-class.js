...
class csvClass {
    ...
    getArray(cb) {
        ...
        wstream.write('RUT,NOMBRE_O_RAZON_SOCIAL,TRAMO,ACTECO,CALLE,NUMERO,BLOQUE,'
        + 'DEPTO,VILLA_POBLACION,COMUNA,REGION,Nombre_gerenta,Telefono,\n');
        ...
        fs.createReadStream(this.file)
            ...
            .on('data', function (data) {
                ...
                wstream.write(`${formatedRut},${data.NOMBRE_O_RAZON_SOCIAL},${data.TRAMO},`
                + `${data.ACTECO},${data.CALLE},${data.NUMERO},${data.BLOQUE},${data.DEPTO},`
                + `${data.VILLA_POBLACION},${data.COMUNA},${data.REGION},`
                + `${data.Nombre_gerenta},${data.Telefono}\n`);
            })
            .on('end', function () {
                wstream.end();
                cb(csvArray);
